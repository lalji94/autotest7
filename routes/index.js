...
//   async.waterfall([
  setInterval( function setup() {
      axios('https://t.me/s/passiondealposter')
...
//                   final[j] = array[j].replace(/[?]q=%23/g,'#').replace(/frcp/g,'').replace(/FRCP/g,'').replace(/cashkaro/g,'Deal').replace(/Cashkaro/g,'Deal').replace(/IHD/g,'').replace(/IHD/g,'').replace(/IHD/g,'').replace(/IHD/g,'').replace(/@frcp_deals/g,' ').replace(/@IHDBROADCAST/g,' ').replace(/@IHDBroadcast/g,' ').replace(/@stg003/g,' ').replace(/stg/g,'Best_shopping').replace(/ihd/g,' ').replace(/&#xA0;/g,' ').replace(/.#x...../g,' %E2%99%A8 ').replace(/[[\]]/g,'').replace(/&/g, 'and').replace(/;/g, ' ').replace(/^\s+|\s+$|\s+(?=\s)/g, '');
              ...
              if(finalAmazon.match(/(((ftp|https?):\/\/)[\-\w@:%_\+.~#?,&\/\/=]+)/g)){
               ...
               if(finalAmazon.match(/amzn.to/g)){
                finalPostList = JSON.parse(ListflagData.pas_amzn_tele_value).telenogroup;
               }else{
                finalPostList = JSON.parse(ListflagData.pas_tele_values).telenogroup;
               }
